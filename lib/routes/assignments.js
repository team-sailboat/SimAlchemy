        ...
        Cohort.findById(assignment.cohortId)
          .then(cohort => {
            ...
            if(assignment.difficulty === 'easy') {
              newSleep = cohort.sleep * 1;
              newKnowledge = cohort.knowledge * .8;
              newStress = cohort.stress * 1;
            }
            else if(assignment.difficulty === 'medium') {
              newSleep = cohort.sleep * .6;
              newKnowledge = cohort.knowledge * 1.5;
              newStress = cohort.stress * 1.4;
            }
            else {
              newSleep = cohort.sleep * .4;
              newKnowledge = cohort.knowledge * 1.2;
              newStress = cohort.stress * 1.6;
            }
            return Promise.all([
